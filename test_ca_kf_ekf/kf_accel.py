...
class KalmanFilterAccel:
    def __init__(self, initial_state, initial_covariance, process_noise_cov, measurement_noise_cov, dt):
        """
        初始化卡尔曼滤波器
        :param initial_state: 初始状态向量 [x, y, vx, vy, ax, ay]
        ...
        # 状态转移矩阵，假设为恒定加速度模型
        self.F = np.array([
            [1, 0, dt, 0, 0.5 * dt**2, 0],  # x 的更新：x_new = x + vx * dt + 0.5 * ax * dt^2
            [0, 1, 0, dt, 0, 0.5 * dt**2],  # y 的更新：y_new = y + vy * dt + 0.5 * ay * dt^2
            [0, 0, 1, 0, dt, 0],            # vx 的更新：vx_new = vx + ax * dt
            [0, 0, 0, 1, 0, dt],            # vy 的更新：vy_new = vy + ay * dt
            [0, 0, 0, 0, 1, 0],             # ax 的更新：ax_new = ax
            [0, 0, 0, 0, 0, 1]              # ay 的更新：ay_new = ay
        ])
